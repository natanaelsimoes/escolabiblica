...
namespace controller\equipe;

use lib\MyCookie;
use lib\util\Pagination;
use model\equipe\Equipe;
...
class EquipeController {
    ...
    public static function manage() {
        ...
        $urlPageNum = filter_input(INPUT_GET, 'page', FILTER_SANITIZE_NUMBER_INT);
        $pageNum = ($urlPageNum) ? $urlPageNum: 1;
        $data = Pagination::paginate(Equipe::select('e'), $pageNum);
        $_MyCookie->loadView('equipe', 'manage'
                , array(
            'equipe' => $data,
            'currentPage' => $pageNum,
